...
function isValidPayload(body: any): body is PrintPayload {
  if (!body) return false;
  if (typeof body.orderNumber !== 'number' && typeof body.orderNumber !== 'string') return false;
...
app.post('/print', async (req, res) => {
  try {
    const body = req.body;
    if (!isValidPayload(body)) return res.status(400).json({ error: 'Payload inválido' });
    ...
    await printToDevice(body as PrintPayload, PRINTER_DEVICE);
