...
app.get('/launcher/health', (_req: Request, res: Response) => {
  res.json({ status: 'ok', service: 'launcher' });
});

let printAgentProcess: ChildProcess | null = null;
...
function startPrintAgent(): Promise<{ success: boolean; message: string }> {
...
app.post('/launcher/start', async (_req: Request, res: Response) => {
  try {
...
app.post('/launcher/stop', (_req: Request, res: Response) => {
  if (printAgentProcess) {
    printAgentProcess.kill();
    printAgentProcess = null;
